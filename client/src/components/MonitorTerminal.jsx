...
const MonitorTerminal = ({ logs, res, isShielded, scrollRef }) => {
  ...
  return (
    ...
    <div className="monitor-terminal flex-[3] bg-black border border-white/10 rounded-[2rem] overflow-hidden shadow-2xl flex flex-col min-h-0 relative group">
      
      {/* 터미널 헤더 바 */}
      <div className="px-4 md:px-8 py-3 md:py-4 border-b border-white/10 flex items-center justify-between bg-white/5 font-mono text-xs text-text-dim">
        <div className="flex items-center gap-2 md:gap-3">
          <Terminal
            size={16}
            className={`transition-colors duration-500 ${isShielded ? 'text-brand-danger' : 'text-brand-primary'}`}
          />
          <span className="tracking-widest uppercase font-black hidden sm:inline">Analysis Stream Output</span>
          <span className="tracking-widest uppercase font-black sm:hidden">Stream</span>
        </div>
      ...
      {/* 로그 출력 영역 — bg-black 고정 (터미널 의도) */}
      <div className="p-4 md:p-8 overflow-y-auto flex-1 font-mono text-xs md:text-sm space-y-3 md:space-y-3.5 custom-scrollbar bg-black text-slate-300 relative">
        {/* 로그가 없을 때의 초기 가이드 */}
        {logs.length === 0 && !res && (
