...
import { Code, Target, Database, Activity, Unlock, FunctionSquare } from 'lucide-react';
  ...
  return (
    <section className="space-y-20">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-border-subtle pt-12 pb-10 mb-12">
        ...
        <div className="flex items-center gap-6 px-8 py-5 bg-bg-input border-2 border-brand-danger/20 rounded-[2rem] shadow-inner group hover:border-brand-danger/40 transition-all min-w-[380px]">
          ...
          <div className="flex flex-col text-left">
            <span className="text-[9px] font-black text-brand-danger uppercase tracking-[0.3em] mb-1">Attack Logic</span>
      ...
      <div className="space-y-0">
        ...
        {/* Phase 01 */}
        <StepRow number="01" icon={<Target size={16} className="text-brand-danger" />}
          title={`Phase_01: ${p.step1.title}`} subLabel={p.step1.desc} accent="danger">
          <InfoPanel>
            <BodyText className="mb-6">{p.step1.detail}</BodyText>
            <AttackVectorsVisualizer selectedId={id} step="step1" />
          </InfoPanel>
        </StepRow>

        {/* Phase 02 */}
        <StepRow number="02" icon={<Database size={16} className="text-brand-danger" />}
          title={`Phase_02: ${p.step2.title}`} subLabel={p.step2.desc} accent="danger">
          <InfoPanel>
            <BodyText className="mb-6">{p.step2.detail}</BodyText>
            <AttackVectorsVisualizer selectedId={id} step="step2" />
          </InfoPanel>
        </StepRow>

        {/* Phase 03 — 핵심 공격 실행 애니메이션 */}
        <StepRow number="03" icon={<Activity size={16} className="text-brand-danger" />}
          title={`Phase_03: ${p.step3.title}`} subLabel={p.step3.desc} accent="danger">
          <InfoPanel>
            <BodyText className="mb-6">{p.step3.detail}</BodyText>
            <div className="w-full bg-bg-card rounded-2xl border border-border-subtle shadow-inner overflow-hidden min-h-[260px] flex items-center justify-center">
              <AttackVectorsVisualizer selectedId={id} step="step3" />
            </div>
          </InfoPanel>
        </StepRow>

        {/* Phase 04 — 결과 */}
        <StepRow number="04" icon={<Unlock size={16} className="text-brand-danger" />}
          title={`Phase_04: ${p.step4.title}`} subLabel={p.step4.desc}
          accent="danger" hasLine={false} last>
          <div className="p-10 bg-bg-card rounded-2xl border-2 border-brand-danger/30 flex flex-col items-center justify-center gap-6 shadow-lg relative overflow-hidden group">
            ...
            <div className="space-y-4 text-center">
              <BodyText variant="dim" className="max-w-md mx-auto italic">{p.step4.detail}</BodyText>
